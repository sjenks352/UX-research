...
intro <- "http://drjim.0catch.com/PracStatPackV2/"
...
#
# data1 and data1 are arrays of matched scores and must be of equal length
# data1 are the y data -- the variable to be predicted; data2 are the x (predictor) data
# conf is the desired level of confidence for the confidence intervals
# predfor is a specific value of x to enter in the regression equation to get an estimate of y
# If not interested in predicting a value for y from x, use 0 (the x-intercept) as a placeholder
analyze.regression <- function(data1,data2,conf,predfor) {
if (conf > 1) conf <- conf/100
confper <- paste(conf*100,"%",sep="")
correl <- cor(data1, data2)
n <- length(data1)
r2t <- correl/((1-correl^2)/(n-2))^.5
df <- n - 2
p <- 2*(1-pt(abs(r2t),df))
zprime <- 0.5*log((1+correl)/(1-correl))
z <- abs(qnorm((1-conf)/2))
critd <- z/(n-3)^.5
upperzprime <- zprime + critd
lowerzprime <- zprime - critd
upper <- (exp(2*upperzprime)-1)/(exp(2*upperzprime)+1)
lower <- (exp(2*lowerzprime)-1)/(exp(2*lowerzprime)+1)
rsquared <- paste(round((correl^2)*100,digits=2),"%",sep="")
rsquaredupper <- paste(round((upper^2)*100,digits=2),"%",sep="")
rsquaredlower <- paste(round((lower^2)*100,digits=2),"%",sep="")
sy <- sd(data1)
sx <- sd(data2)
slope <- correl*sy/sx
bary <- mean(data1)
...
deviations <- (data1 - (slope*data2+intercept))^2
sumdev <- sum(deviations)
xdev <- (data2 - barx)^2
sumxdev <- sum(xdev)
seslope <- (((sumdev)/df)^.5)/(sumxdev^.5)
tcrit <- abs(qt((1-conf)/2,df))
critdslope <- seslope*tcrit
slopeupper <- slope + critdslope
slopelower <- slope - critdslope
seintercept <- ((sumdev/df)^.5)*((1/n)+(barx^2/sumxdev))^.5
critdintercept <- seintercept*tcrit
interceptupper <- intercept + critdintercept
interceptlower <- intercept - critdintercept
predicted <- slope*predfor+intercept
sepredicted <- ((sumdev/df)^.5)*((1/n)+((predfor-barx)^2/sumxdev))^.5
critdpredicted <- sepredicted*tcrit
predictedupper <- predicted + critdpredicted
predictedlower <- predicted - critdpredicted
cat("\nRESULTS\n\n")
cat ("Slope and Intercept","\n")
cat ("Slope:",slope,"\n")
cat ("Intercept:",intercept,"\n","\n")
cat (confper,"Confidence Interval for Slope","\n")
cat ("Standard error:",seslope,"\n")
cat ("Upper limit:", slopeupper, "\n")
cat ("Slope:", slope, "\n")
cat ("Lower limit:", slopelower, "\n\n")
cat (confper,"Confidence Interval for Intercept","\n")
cat ("Standard error:",seintercept,"\n")
cat ("Upper limit:", interceptupper, "\n")
cat ("Intercept:", intercept, "\n")
cat ("Lower limit:", interceptlower, "\n\n")
cat ("Overall Significance of Model","\n")
cat ("r:",correl,"\n")
cat ("df:",df,"\n")
cat ("p:",p,"\n\n")
cat (confper,"Confidence Interval for Correlation","\n")
cat ("Upper limit:", upper, "\n")
cat ("r:", correl, "\n")
cat ("Lower limit:", lower, "\n\n")
cat (confper,"Confidence Interval for R-squared","\n")
cat ("Upper limit:", rsquaredupper, "\n")
cat ("R-squared:", rsquared, "\n")
cat ("Lower limit:", rsquaredlower, "\n\n")
cat (confper,"Confidence Interval for Predicted Value","\n")
cat ("Standard error:",sepredicted,"\n")
cat ("Upper limit:", predictedupper, "\n")
cat ("Predicted value of y given x =",predfor,":", predicted, "\n")
cat ("Lower limit:", predictedlower, "\n\n")
cat ("Values for Sample Size Estimation","\n")
cat ("Mean of x:",barx,"\n")
cat ("Population variability of x:",sumxdev/n,"\n")
cat ("Population variability of error:",sumdev/n,"\n","\n")
cat ("\n")
}
...
# Bonferroni and Benjamini-Hochberg adjustment
#
# n is the planned number of comparisons
# alpha is the overall desired level of of significance
# The value for Bonferroni is alpha divided by the number of comparisons
# The values for Benjamini-Hochberg use their rank-based approach
#
compute.mcadjustments <- function(n,alpha) {
bonferroni <- alpha/n
cat("\nRESULTS\n\n")
cat("Bonferroni adjustment for critical value of p:",bonferroni,"\n\n")
cat("Benjamini-Hochberg adjustments for critical values of p","\n")
iteration <- 1
while (iteration < n+1) {
   bh <- iteration*alpha/n
   rank <- paste("Rank ",iteration,":",sep="")
   cat(rank,bh,"\n")
...
# Sample size estimation for correlation
#
# r is the expected correlation -- if unknown, set to 0
# conf is the desired level of confidence
# critd is the desired width of the resulting confidence interval
n.correlation <- function(r,d,conf) {
if (conf > 1) conf <- conf/100
confper <- paste(conf*100,"%",sep="")
z <- abs(qnorm((1-conf)/2))
n <- ceiling(((z^2*(1-r^2)^2)/d^2)+1+6*r^2)
cat("\nRESULTS\n\n")
cat ("n:",n,"\n")
cat ("\n")
}
...
# Sample size estimation for focus on intercept (or any other prediction)
#
# s2x and s2e are the estimated population variances
# conf is the desired level of confidence (e.g., 95% confidence would be .95 or 95)
# d is the desired magnitude of the resulting margin of error
# predictor is the value of x for the targeted prediction of y
# barx is the mean of the 
# When focusing on the y-intercept, use 0 for the value of "prediction" in the function
# Uses iteration to estimate required sample size, max of 1000 iterations
#
n.prediction <- function(s2x,s2e,d,conf,predictor, barx) {
if (conf > 1) conf <- conf/100
ncurr <- -1
nprev1 <- -2
nprev2 <- -3
iteration <- 0
z <- abs(qnorm((1-conf)/2))
nprev1 <- ceiling((z^2*s2e*(1+(predictor-barx)^2/s2x))/(d^2)+2)
df <- nprev1 - 2
while ((ncurr != nprev1 ) && (ncurr != nprev2) && (iteration < 1001)) {
   ...
   nprev1 <- ncurr
   t <- abs(qt((1-conf)/2,df))
   ncurr <- ceiling((t^2*s2e*(1+(predictor-barx)^2/s2x))/(d^2)+2)
   df <- ncurr - 2
   }
cat("\nRESULTS\n\n")
...
# Sample size estimation for focus on slope given x and e population variances, conf, and d
#
# s2x and s2e are the estimated population variances
# conf is the desired level of confidence (e.g., 95% confidence would be .95 or 95)
# d is the desired magnitude of the resulting margin of error
# Uses iteration to estimate required sample size, max of 1000 iterations
#
n.slope <- function(s2x,s2e,d,conf) {
if (conf > 1) conf <- conf/100
ncurr <- -1
nprev1 <- -2
nprev2 <- -3
iteration <- 0
z <- abs(qnorm((1-conf)/2))
nprev1 <- ceiling((z^2*s2e)/(d^2*s2x)+2)
df <- nprev1 - 2
while ((ncurr != nprev1 ) && (ncurr != nprev2) && (iteration < 1001)) {
   iteration <- iteration + 1
   nprev2 <- nprev1
   nprev1 <- ncurr
   t <- abs(qt((1-conf)/2,df))
   ncurr <- ceiling((t^2*s2e)/(d^2*s2x)+2)
   df <- ncurr - 2
   }
cat("\nRESULTS\n\n")
if (ncurr == nprev2 && ncurr < nprev1) cat("No convergence, fluctuating between:",ncurr,"and",nprev1,"\n")
...
# Correlation magnitude, significance, and confidence interval
#
# data1 and data1 are arrays of matched scores and must be of equal length
# conf is the desired level of confidence
test.correlation <- function(data1,data2,conf) {
if (conf > 1) conf <- conf/100
confper <- paste(conf*100,"%",sep="")
correl <- cor(data1, data2)
n <- length(data1)
...
# Phi correlation magnitude, significance, and confidence interval
#
# data1 and data1 are arrays of matched scores and must be of equal length
# conf is the desired level of confidence
test.phi <- function(a,b,c,d,conf) {
if (conf > 1) conf <- conf/100
confper <- paste(conf*100,"%",sep="")
phi <- (a*d-b*c)/((a+b)*(c+d)*(a+c)*(b+d))^.5
n <- a+b+c+d
chi <- n*phi^2
df <- 1
p <- 1-(pchisq(chi,df))
zprime <- 0.5*log((1+phi)/(1-phi))
z <- abs(qnorm((1-conf)/2))
critd <- z/(n-3)^.5
upperzprime <- zprime + critd
lowerzprime <- zprime - critd
